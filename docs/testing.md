...
The Node test runner executes 460 named tests. The production HTTP suite adds 101 assertions against a real Next.js production server. The browser suite adds a complete interaction journey with 21 explicit assertions plus locator and state-transition checks that fail on missing or inaccessible UI.
