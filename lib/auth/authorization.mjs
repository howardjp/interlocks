...
const MEMBER_ACTIONS = new Set([ACTIONS.READ_WORKSPACE, ACTIONS.READ_PERSONAL_LEDGER, ACTIONS.VIEW_DOCUMENT]);
...
export class AuthorizationService {
  can(actor, action, { workspaceId = null, ownerPersonId = null, readOnly = false } = {}) {
    if (!actor || actor.accountStatus !== "ACTIVE") return false;
    if (actor.platformRole === "SUPERADMIN") return readOnly || action !== ACTIONS.SHARE_PERSONAL_LEDGER || actor.personId === ownerPersonId;
    if (ownerPersonId && actor.personId === ownerPersonId && [ACTIONS.READ_PERSONAL_LEDGER, ACTIONS.SHARE_PERSONAL_LEDGER].includes(action)) return true;
