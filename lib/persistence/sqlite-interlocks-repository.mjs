...
function normalizePolicyContext(value) {
  ...
  if (!isObject(value)) throw new Error("Policy question context must be an object");
  const allowed = new Set(["tribunal", "delawareCounselConfirmed", "outsideCounselPresent", "proHacViceStatus"]);
  for (const key of Object.keys(value)) if (!allowed.has(key)) throw new Error(`Unsupported policy context fact: ${key}`);
  for (const key of ["delawareCounselConfirmed", "outsideCounselPresent"]) if (key in value && typeof value[key] !== "boolean") throw new Error(`${key} must be a boolean`);
  if ("tribunal" in value && value.tribunal !== "DELAWARE_CHANCERY") throw new Error("Unsupported tribunal policy context");
  if ("proHacViceStatus" in value && !["ACTIVE", "PENDING", "DENIED", "EXPIRED", "NOT_STARTED"].includes(value.proHacViceStatus)) throw new Error("Unsupported pro hac vice status");
...
export class SqliteInterlocksRepository extends InterlocksRepository {
  ...
  getSnapshot(accountId = "acct-alex", requestedWorkspaceId = null, options = {}) {
    ...
    const policyPacks = this.database.prepare(`SELECT pack_id AS id,version,title,short_title AS shortTitle,authority_type AS authorityType,jurisdiction,publisher,
      ...
      manifest_json AS manifestJson FROM policy_packs ORDER BY CASE authority_type WHEN 'MODEL' THEN 1 WHEN 'LICENSING_JURISDICTION' THEN 2 ELSE 3 END,title`).all().map((item) => ({ ...item, manifest:parse(item.manifestJson, {}) }));
  ...
  recordPolicyQuestion(accountId, checkId, question, baseFacts, at) {
    ...
    for (const authority of question.authorities) {
      ...
      const facts = { ...structuredClone(baseFacts), ...structuredClone(question.context) };
  ...
  createConflictCheck(accountId, workspaceId, input) {
    ...
    transaction(this.database, () => {
      ...
      this.database.prepare("UPDATE conflict_checks SET workflow_state=?,knowledge_snapshot_json=? WHERE id=?").run(createdHits.length || policyActionCount ? "YELLOW" : "GREEN", JSON.stringify({ corpusRevision:revision, entityIds:[...visible], policyFactsHash:policyContentHash(baseFacts), policyQuestionKeys:policyQuestions.map((item) => item.key) }), checkId);
  ...
  insertHit(checkId, subjectId, matchedEntityId, workspaceId, result, at, sourceType = null, sourceId = null) {
    ...
    return { id: hitId, subjectId, matchedEntityId, matchedEntityName: entity.name, matchConfidence: result.confidence, sourceResourceType:effectiveSourceType, sourceResourceId:effectiveSourceId, explanation };
  }
  ...
  resetDemo(accountId = "acct-alex") {
    ...
    const tables = ["audit_events","seat_events","import_batches","associated_person_responses","associated_person_requests","controls","screens","conflict_consents","human_determinations","review_notes","review_cases","policy_rule_results","policy_evaluations","policy_authority_selections","policy_questions","conflict_hit_evidence","conflict_hits","conflict_check_subjects","conflict_checks","evidence_links","resource_attachments","documents","inferences","assertions","personal_ledger_entries","professional_relationships","matter_parties","matters","invitations","workspace_roles","workspace_memberships","workspaces","entity_relationships","entity_addresses","entity_identifiers","entity_aliases","entities","auth_identities","accounts","persons"];
    transaction(this.database, () => {
