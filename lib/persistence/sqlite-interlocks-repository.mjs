...
} from "../domain/family-associations.mjs";
...
import { AUTHORITY_STATUSES, evaluatePolicyPack, policyContentHash } from "../policy/policy-engine.mjs";
import { LEGAL_POLICY_PACKS, getLegalPolicyPack } from "../policy/legal-policy-packs.mjs";
import { DERIVED_CONFLICT_FACTS, validateLegalConflictContext } from "../policy/legal-conflicts-schema.mjs";
...
const RESOURCE_TABLES = Object.freeze({
  ...
  POLICY_QUESTION: "policy_questions", POLICY_EVALUATION: "policy_evaluations",
  ASSOCIATED_PERSON_REQUEST: "associated_person_requests", IMPORT_BATCH: "import_batches",
  PERSONAL_ASSOCIATION: "personal_associations", ASSOCIATION_INTEREST: "personal_association_interests",
  FAMILY_ACCOUNT_LINK: "family_account_links",
...
function optionalDate(value, label) { const result = clean(value); if (!result) return null; if (!/^\d{4}-\d{2}-\d{2}$/.test(result) || !Number.isFinite(Date.parse(`${result}T00:00:00Z`))) throw new Error(`${label} must be a valid date`); return result; }
function dateWindow(from, to) { const start = optionalDate(from, "Effective-from date"); const end = optionalDate(to, "Effective-to date"); if (start && end && start >= end) throw new Error("Effective-to date must be after effective-from date"); return { start, end }; }

function normalizePolicyContext(value) {
  return validateLegalConflictContext(value);
}
...
export class SqliteInterlocksRepository extends InterlocksRepository {
  ...
  syncPolicyPacks() {
    const insert = this.database.prepare(`INSERT INTO policy_packs
    ...
    for (const pack of LEGAL_POLICY_PACKS) {
      const existing = this.database.prepare("SELECT content_hash AS contentHash FROM policy_packs WHERE pack_id=? AND version=?").get(pack.id, pack.version);
      if (existing && existing.contentHash !== pack.contentHash) throw new Error(`Installed policy pack changed without a version increment: ${pack.id}`);
      if (!existing) insert.run(pack.id, pack.version, pack.title, pack.shortTitle || null, pack.authorityType, pack.jurisdiction || null, pack.publisher, pack.effectiveFrom, pack.effectiveTo || null, pack.status, pack.sourceUrl, pack.description || null, pack.dslVersion, pack.contentHash, JSON.stringify(pack), isoNow());
    }
  }

  normalizePolicyQuestions(input = {}) {
    const raw = input.questions?.length ? input.questions : [{
      key: "general-conflicts-screen", text: input.question || "What conflicts or professional duties should be reviewed before this matter proceeds?",
      authorities: input.authorities || [], context: input.policyContext || {},
    }];
    if (raw.length > 12) throw new Error("A conflict check may contain at most 12 policy questions");
    const keys = new Set();
    return raw.map((question, index) => {
      const key = clean(question.key) || `question-${index + 1}`;
      if (keys.has(key)) throw new Error("Policy question keys must be unique within a conflict check");
      keys.add(key);
      const selected = new Map();
      for (const authority of question.authorities || []) {
        const packId = required(authority.packId, "Policy pack");
        const status = upper(authority.status || "POTENTIALLY_APPLICABLE");
        getLegalPolicyPack(packId);
        if (!AUTHORITY_STATUSES.includes(status)) throw new Error("Unsupported authority status");
        if (selected.has(packId)) throw new Error(`Policy pack selected more than once for question: ${packId}`);
        if (packId === "aba-model" && status === "CONTROLLING") throw new Error("The ABA Model Rules may be provisional or comparative, but are not controlling law");
        selected.set(packId, { packId, status, source: upper(authority.source || "USER"), rationale: clean(authority.rationale) || null });
      }
      const hasControlling = [...selected.values()].some((item) => item.status === "CONTROLLING");
      if (!selected.has("aba-model")) selected.set("aba-model", { packId:"aba-model", status:hasControlling ? "COMPARATIVE_ONLY" : "POTENTIALLY_APPLICABLE", source:"SYSTEM_FALLBACK", rationale:hasControlling ? "Permanent comparison baseline" : "First-blush screening while governing authority remains unresolved" });
      if (selected.has("delaware-chancery") && !selected.has("delaware")) selected.set("delaware", { packId:"delaware", status:"POTENTIALLY_APPLICABLE", source:"SYSTEM_FALLBACK", rationale:"Chancery is a tribunal overlay that operates alongside Delaware professional-conduct rules" });
      return { key, text: required(question.text || question.question, "Policy question"), context: normalizePolicyContext(question.context), authorities: [...selected.values()] };
    });
  }
  ...
  seedDemo() {
    ...
    transaction(db, () => {
      ...
      db.prepare("INSERT INTO persons (id,display_name,primary_profession,created_at,updated_at) VALUES ('p-zoe-chen','Zoe Chen','Technology executive',?,?)").run(at, at);
      db.prepare("INSERT INTO entities (id,kind,canonical_name,person_id,created_at,updated_at) VALUES ('ent-p-zoe-chen','PERSON','Zoe Chen','p-zoe-chen',?,?)").run(at, at);
      ...
      const organizations = [
        ...
        ["o-microsoft", "Microsoft Corporation", "Washington, US", "ORGANIZATION"],
        ["o-aperture", "Aperture Technologies", "California, US", "ORGANIZATION"],
      ...
      db.prepare("INSERT INTO personal_ledger_entries (id,person_id,entity_id,context,involvement,source,provenance,disclosure_class,sharing_authorized,recorded_at) VALUES ('ledger-nina-aperture','p-nina','o-aperture','Independent board advisory role','BOARD_ADVISER','Self-disclosed','Account holder authorized identity-only matching','PORTABLE',1,?)").run(at);
      db.prepare("INSERT INTO personal_associations (id,owner_person_id,associated_person_id,relationship_type,disclosure_scope,status,effective_from,provenance,created_by,created_at,updated_at) VALUES ('assoc-maya-zoe','p-maya','p-zoe-chen','CHILD','MATCH_AND_RELATIONSHIP','ACTIVE','2001-01-01','Account holder declaration','acct-maya',?,?)").run(at, at);
      db.prepare("INSERT INTO personal_association_interests (id,association_id,entity_id,involvement,description,status,sharing_authorized,effective_from,recorded_by,recorded_at) VALUES ('assoc-interest-zoe-microsoft','assoc-maya-zoe','o-microsoft','VICE_PRESIDENT','Vice president at Microsoft Corporation','CURRENT',1,'2025-01-01','acct-maya',?)").run(at);
      db.prepare("INSERT INTO family_account_links (id,pair_key,requester_person_id,target_person_id,relationship_type,target_relationship_type,disclosure_scope,status,requested_by,requested_at,expires_at,responded_by,responded_at,accepted_at) VALUES ('family-link-alex-nina','p-alex::p-nina','p-alex','p-nina','SPOUSE','SPOUSE','ENTITY_MATCH_ONLY','ACTIVE','acct-alex',?,'2036-08-28T12:00:00.000Z','acct-nina',?,?)").run(at, at, at);
  ...
  getSnapshot(accountId = "acct-alex", requestedWorkspaceId = null, options = {}) {
    ...
    const checks = query(`SELECT id,reference,matter_id AS matterId,status,workflow_state AS workflowState,executed_at AS executedAt,corpus_revision AS corpusRevision,(corpus_revision < ?) AS reReviewSuggested,
      (SELECT COUNT(*) FROM conflict_hits WHERE conflict_check_id=conflict_checks.id) AS hitCount,
      (SELECT COUNT(*) FROM policy_questions WHERE conflict_check_id=conflict_checks.id) AS policyQuestionCount,
      (SELECT COUNT(*) FROM policy_rule_results pr JOIN policy_evaluations pe ON pe.id=pr.evaluation_id JOIN policy_questions pq ON pq.id=pe.question_id WHERE pq.conflict_check_id=conflict_checks.id AND pr.outcome!='NOT_MATCHED') AS policyFindingCount
      FROM conflict_checks WHERE workspace_id=? ORDER BY executed_at DESC`, this.corpusRevision(), workspaceId).map((item) => ({ ...item, reReviewSuggested: Boolean(item.reReviewSuggested) }));
    ...
    const policyPacks = this.database.prepare(`SELECT pack_id AS id,version,title,short_title AS shortTitle,authority_type AS authorityType,jurisdiction,publisher,
      effective_from AS effectiveFrom,effective_to AS effectiveTo,status,source_url AS sourceUrl,description,dsl_version AS dslVersion,content_hash AS contentHash,
      manifest_json AS manifestJson FROM policy_packs p WHERE p.rowid=(SELECT newest.rowid FROM policy_packs newest WHERE newest.pack_id=p.pack_id ORDER BY newest.installed_at DESC,newest.rowid DESC LIMIT 1)
      ORDER BY CASE authority_type WHEN 'MODEL' THEN 1 WHEN 'LICENSING_JURISDICTION' THEN 2 ELSE 3 END,title`).all().map((item) => ({ ...item, manifest:parse(item.manifestJson, {}) }));
    const policyQuestions = query(`SELECT q.id,q.conflict_check_id AS conflictCheckId,q.question_key AS questionKey,q.question_text AS questionText,q.scope,q.created_at AS createdAt
      FROM policy_questions q JOIN conflict_checks c ON c.id=q.conflict_check_id WHERE c.workspace_id=? ORDER BY q.created_at`, workspaceId);
    const policySelections = query(`SELECT s.id,s.question_id AS questionId,s.pack_id AS packId,s.pack_version AS packVersion,s.authority_status AS authorityStatus,
      s.selection_source AS selectionSource,s.rationale,s.selected_at AS selectedAt FROM policy_authority_selections s
      JOIN policy_questions q ON q.id=s.question_id JOIN conflict_checks c ON c.id=q.conflict_check_id WHERE c.workspace_id=? ORDER BY s.selected_at,s.pack_id`, workspaceId);
    const policyEvaluations = query(`SELECT pe.id,pe.question_id AS questionId,pe.authority_selection_id AS authoritySelectionId,pe.engine_version AS engineVersion,
      pe.dsl_version AS dslVersion,pe.facts_hash AS factsHash,pe.summary_json AS summaryJson,pe.evaluated_at AS evaluatedAt
      FROM policy_evaluations pe JOIN policy_questions q ON q.id=pe.question_id JOIN conflict_checks c ON c.id=q.conflict_check_id
      WHERE c.workspace_id=? ORDER BY pe.evaluated_at`, workspaceId).map((item) => ({ ...item, summary:parse(item.summaryJson, {}) }));
    const policyRuleResults = query(`SELECT pr.id,pr.evaluation_id AS evaluationId,pr.pack_id AS packId,pr.pack_version AS packVersion,pr.rule_id AS ruleId,
      pr.corresponds_to AS correspondsTo,pr.outcome,pr.finding_code AS findingCode,pr.finding_message AS findingMessage,pr.citation,pr.source_url AS sourceUrl,
      pr.comparison_note AS comparisonNote,pr.missing_facts_json AS missingFactsJson,pr.unknown_questions_json AS unknownQuestionsJson,pr.trace_json AS traceJson
      FROM policy_rule_results pr JOIN policy_evaluations pe ON pe.id=pr.evaluation_id JOIN policy_questions q ON q.id=pe.question_id
      JOIN conflict_checks c ON c.id=q.conflict_check_id WHERE c.workspace_id=? ORDER BY pr.pack_id,pr.rule_id`, workspaceId).map((item) => ({ ...item, missingFacts:parse(item.missingFactsJson, []), unknownQuestions:parse(item.unknownQuestionsJson, []), trace:parse(item.traceJson, {}) }));
    ...
    const personalOwnerId = viewAs ? null : actor.personId;
    const ledger = personalOwnerId ? this.database.prepare(`SELECT l.id,l.person_id AS personId,e.canonical_name AS entityName,l.context,l.involvement,l.source_workspace_id AS sourceWorkspaceId,l.source,l.provenance,l.disclosure_class AS disclosureClass,l.sharing_authorized AS sharingAuthorized,l.recorded_at AS recordedAt FROM personal_ledger_entries l JOIN entities e ON e.id=l.entity_id WHERE l.person_id=? ORDER BY l.recorded_at DESC`).all(personalOwnerId).map((item) => ({ ...item, sharingAuthorized: Boolean(item.sharingAuthorized) })) : [];
    const personalAssociations = personalOwnerId ? this.database.prepare(`SELECT pa.id,pa.owner_person_id AS ownerPersonId,pa.associated_person_id AS associatedPersonId,
      p.display_name AS associatedPersonName,EXISTS(SELECT 1 FROM accounts a WHERE a.person_id=pa.associated_person_id AND a.status='ACTIVE') AS associatedPersonHasAccount,
      pa.relationship_type AS relationshipType,pa.disclosure_scope AS disclosureScope,pa.status,pa.effective_from AS effectiveFrom,
      pa.effective_to AS effectiveTo,pa.provenance,pa.created_at AS createdAt,pa.updated_at AS updatedAt
      FROM personal_associations pa JOIN persons p ON p.id=pa.associated_person_id WHERE pa.owner_person_id=? ORDER BY pa.status,p.display_name`).all(personalOwnerId).map((item) => ({ ...item, associatedPersonHasAccount:Boolean(item.associatedPersonHasAccount) })) : [];
    const associationInterests = personalOwnerId ? this.database.prepare(`SELECT pai.id,pai.association_id AS associationId,pa.associated_person_id AS associatedPersonId,
      p.display_name AS associatedPersonName,pai.entity_id AS entityId,e.canonical_name AS entityName,pai.involvement,pai.description,pai.status,
      pai.sharing_authorized AS sharingAuthorized,pai.effective_from AS effectiveFrom,pai.effective_to AS effectiveTo,pai.recorded_at AS recordedAt,pai.revoked_at AS revokedAt
      FROM personal_association_interests pai JOIN personal_associations pa ON pa.id=pai.association_id JOIN persons p ON p.id=pa.associated_person_id
      JOIN entities e ON e.id=pai.entity_id WHERE pa.owner_person_id=? ORDER BY pai.status,pai.recorded_at DESC`).all(personalOwnerId).map((item) => ({ ...item, sharingAuthorized:Boolean(item.sharingAuthorized) })) : [];
    const familyAccountLinks = personalOwnerId ? this.database.prepare(`SELECT fl.id,fl.requester_person_id AS requesterPersonId,fl.target_person_id AS targetPersonId,
      CASE WHEN fl.requester_person_id=? THEN fl.target_person_id ELSE fl.requester_person_id END AS otherPersonId,
      CASE WHEN fl.requester_person_id=? THEN target.display_name ELSE requester.display_name END AS otherPersonName,
      CASE WHEN fl.requester_person_id=? THEN fl.relationship_type ELSE fl.target_relationship_type END AS relationshipType,
      CASE WHEN fl.requester_person_id=? THEN 'SENT' ELSE 'RECEIVED' END AS direction,fl.disclosure_scope AS disclosureScope,fl.status,
      fl.requested_at AS requestedAt,fl.expires_at AS expiresAt,fl.accepted_at AS acceptedAt,fl.revoked_at AS revokedAt
      FROM family_account_links fl JOIN persons requester ON requester.id=fl.requester_person_id JOIN persons target ON target.id=fl.target_person_id
      WHERE fl.requester_person_id=? OR fl.target_person_id=? ORDER BY fl.requested_at DESC`).all(personalOwnerId, personalOwnerId, personalOwnerId, personalOwnerId, personalOwnerId, personalOwnerId).map((item) => {
        const expired = item.status === "PENDING" && item.expiresAt && new Date(item.expiresAt) <= new Date();
        const status = expired ? "EXPIRED" : item.status;
        return { ...item, status, canRespond:item.direction === "RECEIVED" && status === "PENDING", canRevoke:status === "ACTIVE" };
      }) : [];
    return {
      ...
      assertions, inferences, policyPacks, policyQuestions, policySelections, policyEvaluations, policyRuleResults,
      documents, memberships, invitations, associatedRequests, associatedResponses, audit, ledger,
      personalAssociations, associationInterests, familyAccountLinks,
  ...
  updateMembership(accountId, workspaceId, membershipId, input) {
    ...
    transaction(this.database, () => {
      ...
      if (input.status && upper(input.status) !== membership.status) {
        ...
        if (membership.status !== "ACTIVE" && status === "ACTIVE") this.database.prepare("INSERT INTO seat_events (id,workspace_id,membership_id,delta,reason,effective_at) VALUES (?,?,?,1,'Member reactivated',?)").run(id("seat"), workspaceId, membershipId, at);
  ...
  createAssertion(accountId, workspaceId, input) {
    ...
    if (input.supersedesId && !this.database.prepare("SELECT 1 FROM assertions WHERE id=? AND workspace_id=?").get(input.supersedesId, workspaceId)) throw new Error("Superseded assertion not found");
    transaction(this.database, () => {
  ...
  uploadDocument(accountId, workspaceId, input) {
    ...
    const filename = required(input.filename, "Filename"); const mediaType = required(input.mediaType, "Media type");
    if (input.supersedesDocumentId && !this.database.prepare("SELECT 1 FROM documents WHERE id=? AND workspace_id=?").get(input.supersedesDocumentId, workspaceId)) throw new Error("Superseded document not found in this workspace");
    for (const attachment of input.attachments || []) this.assertResource(attachment.resourceType, attachment.resourceId, workspaceId);
    for (const evidence of input.evidenceLinks || []) this.assertEvidence(evidence.assertionId, workspaceId);
    const stored = this.objectStore.putImmutable(bytes); const documentId = id("document"); const at = isoNow();
    transaction(this.database, () => {
      this.database.prepare("INSERT INTO documents (id,workspace_id,filename,media_type,size,sha256,storage_key,uploaded_by,uploaded_at,description,confidentiality_scope,status,supersedes_document_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,'CURRENT',?)").run(documentId, workspaceId, filename, mediaType, stored.size, stored.sha256, stored.storageKey, accountId, at, clean(input.description) || null, upper(input.confidentialityScope || "WORKSPACE"), clean(input.supersedesDocumentId) || null);
      ...
      for (const attachment of input.attachments || []) this.attachDocumentInternal(documentId, attachment, accountId, workspaceId, at);
      for (const evidence of input.evidenceLinks || []) this.linkEvidenceInternal(documentId, evidence, accountId, workspaceId, at);
  ...
  assertResource(resourceType, resourceId, workspaceId) {
    ...
    const directWorkspaceTables = {
      MEMBERSHIP: "workspace_memberships", MATTER: "matters", RELATIONSHIP: "professional_relationships",
      ASSERTION: "assertions", INFERENCE: "inferences", CONFLICT_CHECK: "conflict_checks",
      REVIEW_CASE: "review_cases", ASSOCIATED_PERSON_REQUEST: "associated_person_requests", IMPORT_BATCH: "import_batches",
    };
    let visible = false;
    if (type === "WORKSPACE") visible = resourceId === workspaceId;
    else if (directWorkspaceTables[type]) visible = Boolean(this.database.prepare(`SELECT 1 FROM ${directWorkspaceTables[type]} WHERE id=? AND workspace_id=?`).get(resourceId, workspaceId));
    else if (type === "PERSON") visible = Boolean(this.database.prepare("SELECT 1 FROM workspace_memberships WHERE person_id=? AND workspace_id=? UNION SELECT 1 FROM review_cases WHERE person_id=? AND workspace_id=?").get(resourceId, workspaceId, resourceId, workspaceId));
    else if (type === "ACCOUNT") visible = Boolean(this.database.prepare("SELECT 1 FROM workspace_memberships WHERE account_id=? AND workspace_id=? UNION SELECT 1 FROM review_cases WHERE assigned_account_id=? AND workspace_id=?").get(resourceId, workspaceId, resourceId, workspaceId));
    else if (type === "ENTITY") visible = Boolean(this.database.prepare(`SELECT 1 FROM workspaces WHERE organization_entity_id=? AND id=?
      UNION SELECT 1 FROM professional_relationships WHERE entity_id=? AND workspace_id=?
      UNION SELECT 1 FROM matter_parties mp JOIN matters m ON m.id=mp.matter_id WHERE mp.entity_id=? AND m.workspace_id=?
      UNION SELECT 1 FROM review_cases WHERE entity_id=? AND workspace_id=?`).get(resourceId, workspaceId, resourceId, workspaceId, resourceId, workspaceId, resourceId, workspaceId));
    else if (type === "CONFLICT_HIT") visible = Boolean(this.database.prepare("SELECT 1 FROM conflict_hits h JOIN conflict_checks c ON c.id=h.conflict_check_id WHERE h.id=? AND c.workspace_id=?").get(resourceId, workspaceId));
    else if (type === "DETERMINATION") visible = Boolean(this.database.prepare("SELECT 1 FROM human_determinations d JOIN review_cases c ON c.id=d.case_id WHERE d.id=? AND c.workspace_id=?").get(resourceId, workspaceId));
    else if (type === "CONSENT") visible = Boolean(this.database.prepare("SELECT 1 FROM conflict_consents x JOIN review_cases c ON c.id=x.case_id WHERE x.id=? AND c.workspace_id=?").get(resourceId, workspaceId));
    else if (type === "SCREEN") visible = Boolean(this.database.prepare("SELECT 1 FROM screens s JOIN review_cases c ON c.id=s.case_id WHERE s.id=? AND c.workspace_id=?").get(resourceId, workspaceId));
    else if (type === "CONTROL") visible = Boolean(this.database.prepare("SELECT 1 FROM controls x JOIN review_cases c ON c.id=x.case_id WHERE x.id=? AND c.workspace_id=?").get(resourceId, workspaceId));
    else if (type === "POLICY_QUESTION") visible = Boolean(this.database.prepare("SELECT 1 FROM policy_questions q JOIN conflict_checks c ON c.id=q.conflict_check_id WHERE q.id=? AND c.workspace_id=?").get(resourceId, workspaceId));
    else if (type === "POLICY_EVALUATION") visible = Boolean(this.database.prepare("SELECT 1 FROM policy_evaluations pe JOIN policy_questions q ON q.id=pe.question_id JOIN conflict_checks c ON c.id=q.conflict_check_id WHERE pe.id=? AND c.workspace_id=?").get(resourceId, workspaceId));
    else if (type === "ADMINISTRATIVE_ACTION") visible = Boolean(this.database.prepare("SELECT 1 FROM audit_events WHERE id=? AND workspace_scope=?").get(resourceId, workspaceId));
    if (!visible) throw new Error("Attachment target crosses the workspace boundary");
    return type;
  }

  assertEvidence(assertionId, workspaceId) {
    if (!this.database.prepare("SELECT 1 FROM assertions WHERE id=? AND workspace_id=?").get(assertionId, workspaceId)) throw new Error("Evidence assertion crosses the workspace boundary");
  }

  attachDocumentInternal(documentId, attachment, accountId, workspaceId, at = isoNow()) {
    if (!this.database.prepare("SELECT 1 FROM documents WHERE id=? AND workspace_id=?").get(documentId, workspaceId)) throw new Error("Document crosses the workspace boundary");
    const resourceType = this.assertResource(attachment.resourceType, attachment.resourceId, workspaceId);
  ...
  linkEvidenceInternal(documentId, evidence, accountId, workspaceId, at = isoNow()) {
    if (!this.database.prepare("SELECT 1 FROM documents WHERE id=? AND workspace_id=?").get(documentId, workspaceId)) throw new Error("Document crosses the workspace boundary");
    this.assertEvidence(evidence.assertionId, workspaceId);
  ...
  visibleEntityCandidates(workspaceId, actor, participatingPersonIds = []) {
    this.authorization.require(actor, ACTIONS.READ_WORKSPACE, { workspaceId });
    const candidates = new Map();
    const add = (entityId, source) => {
      if (!entityId) return;
      const candidate = candidates.get(entityId) || { sources:[] };
      if (!candidate.sources.some((item) => item.sourceType === source.sourceType && item.sourceId === source.sourceId)) candidate.sources.push(source);
      candidate.sources.sort((left, right) => left.priority - right.priority || left.sourceType.localeCompare(right.sourceType) || left.sourceId.localeCompare(right.sourceId));
      candidates.set(entityId, candidate);
    };
    for (const row of this.database.prepare("SELECT entity_id AS entityId,id AS sourceId FROM professional_relationships WHERE workspace_id=?").all(workspaceId)) add(row.entityId, { sourceType:"RELATIONSHIP", sourceId:row.sourceId, private:false, priority:10 });
    for (const row of this.database.prepare("SELECT mp.entity_id AS entityId,mp.id AS sourceId FROM matter_parties mp JOIN matters m ON m.id=mp.matter_id WHERE m.workspace_id=?").all(workspaceId)) add(row.entityId, { sourceType:"MATTER_PARTY", sourceId:row.sourceId, private:false, priority:20 });
    const organization = this.database.prepare("SELECT organization_entity_id AS entityId FROM workspaces WHERE id=?").get(workspaceId);
    add(organization?.entityId, { sourceType:"WORKSPACE", sourceId:workspaceId, private:false, priority:30 });
    for (const row of this.database.prepare(`SELECT q.query_entity_id AS entityId,r.id AS sourceId,r.response FROM associated_person_responses r
      JOIN associated_person_requests q ON q.id=r.request_id WHERE q.workspace_id=? AND q.status='ANSWERED' AND r.response!='NO_KNOWN_CONNECTION'`).all(workspaceId)) add(row.entityId, { sourceType:"ASSOCIATED_PERSON_RESPONSE", sourceId:row.sourceId, private:false, priority:40 });

    const coveredPeople = new Set([actor.personId]);
    for (const personId of participatingPersonIds) if (this.database.prepare("SELECT 1 FROM workspace_memberships WHERE workspace_id=? AND person_id=? AND status='ACTIVE'").get(workspaceId, personId)) coveredPeople.add(personId);
    for (const personId of coveredPeople) {
      for (const entry of this.database.prepare("SELECT id,entity_id AS entityId,disclosure_class AS disclosureClass,sharing_authorized AS sharingAuthorized FROM personal_ledger_entries WHERE person_id=?").all(personId)) {
        if (linkedLedgerEntryIsShareable(entry)) add(entry.entityId, { sourceType:"LEDGER_ENTRY", sourceId:entry.id, private:true, priority:50 });
      }
      for (const row of this.database.prepare(`SELECT pa.id AS associationId,pa.relationship_type AS relationshipType,pa.status AS associationStatus,
        pa.disclosure_scope AS disclosureScope,pa.effective_from AS associationEffectiveFrom,pa.effective_to AS associationEffectiveTo,pai.id AS interestId,pai.entity_id AS entityId,
        pai.involvement,pai.status AS interestStatus,pai.sharing_authorized AS interestSharingAuthorized,pai.effective_from AS interestEffectiveFrom,
        pai.effective_to AS interestEffectiveTo FROM personal_associations pa JOIN personal_association_interests pai ON pai.association_id=pa.id
        WHERE pa.owner_person_id=?`).all(personId)) {
        const association = { status:row.associationStatus, effectiveFrom:row.associationEffectiveFrom, effectiveTo:row.associationEffectiveTo };
        const interest = { status:row.interestStatus, sharingAuthorized:Boolean(row.interestSharingAuthorized), effectiveFrom:row.interestEffectiveFrom, effectiveTo:row.interestEffectiveTo };
        if (associationIsEffective(association) && associationInterestIsEffective(interest)) add(row.entityId, {
          sourceType:"DECLARED_FAMILY_INTEREST", sourceId:row.interestId, private:true, priority:60,
          sourceDetail:familyCandidateReason({ mode:"DECLARED", relationshipType:row.relationshipType, involvement:row.involvement, discloseRelationship:row.disclosureScope === "MATCH_AND_RELATIONSHIP" }),
        });
      }
      for (const link of this.database.prepare("SELECT * FROM family_account_links WHERE requester_person_id=? OR target_person_id=?").all(personId, personId)) {
        if (!familyLinkIsEffective(link)) continue;
        const requester = link.requester_person_id === personId;
        const otherPersonId = requester ? link.target_person_id : link.requester_person_id;
        const relationshipType = requester ? link.relationship_type : link.target_relationship_type;
        for (const entry of this.database.prepare("SELECT id,entity_id AS entityId,disclosure_class AS disclosureClass,sharing_authorized AS sharingAuthorized FROM personal_ledger_entries WHERE person_id=?").all(otherPersonId)) {
          if (!linkedLedgerEntryIsShareable(entry)) continue;
          add(entry.entityId, { sourceType:"FAMILY_ACCOUNT_LINK", sourceId:link.id, private:true, priority:70, sourceDetail:familyCandidateReason({ mode:"ACCOUNT_LINKED", relationshipType }) });
        }
      }
    }

    const directlyVisible = new Set(candidates.keys());
    for (const relation of this.database.prepare("SELECT id,from_entity_id AS fromId,to_entity_id AS toId FROM entity_relationships").all()) {
      if (directlyVisible.has(relation.fromId) && candidates.get(relation.fromId).sources.some((source) => !source.private)) add(relation.toId, { sourceType:"ENTITY_RELATIONSHIP", sourceId:relation.id, private:false, priority:80 });
      if (directlyVisible.has(relation.toId) && candidates.get(relation.toId).sources.some((source) => !source.private)) add(relation.fromId, { sourceType:"ENTITY_RELATIONSHIP", sourceId:relation.id, private:false, priority:80 });
    }
    return candidates;
  }

  visibleEntityIds(workspaceId, actor, participatingPersonIds = []) {
    return new Set(this.visibleEntityCandidates(workspaceId, actor, participatingPersonIds).keys());
  }

  buildPolicyFacts(workspaceId, input, subjects, hits, at) {
    const matter = input.matterId ? this.database.prepare("SELECT id,code,title,matter_type AS matterType,stage,status,representation_status AS representationStatus FROM matters WHERE id=? AND workspace_id=?").get(input.matterId, workspaceId) : null;
    const matterParties = matter ? this.database.prepare("SELECT mp.id,mp.entity_id AS entityId,e.canonical_name AS entityName,mp.role,mp.status FROM matter_parties mp JOIN entities e ON e.id=mp.entity_id WHERE mp.matter_id=?").all(matter.id) : [];
    const indicators = []; const connections = []; const indicatorKeys = new Set();
    const addIndicator = (type, hit, evidenceType, evidenceId, detail) => {
      const key = `${type}:${hit.subjectId}:${hit.matchedEntityId}:${evidenceType}:${evidenceId}`;
      if (indicatorKeys.has(key)) return;
      indicatorKeys.add(key);
      indicators.push({ type, subjectId:hit.subjectId, entityId:hit.matchedEntityId, entityName:hit.matchedEntityName, evidenceType, evidenceId, detail });
    };
    const subjectById = new Map(hits.map((hit) => [hit.subjectId, subjects.find((subject) => subject.__policySubjectId === hit.subjectId) || null]));
    for (const hit of hits) {
      const subject = subjectById.get(hit.subjectId) || {};
      const subjectRole = upper(subject.role || "OTHER");
      const evidenceSources = hit.__evidenceSources || [{ sourceType:hit.sourceResourceType, sourceId:hit.sourceResourceId }];
      for (const source of evidenceSources.filter((item) => ["DECLARED_FAMILY_INTEREST","FAMILY_ACCOUNT_LINK"].includes(item.sourceType))) addIndicator("FAMILY_CONNECTION", hit, source.sourceType, hit.id, source.sourceDetail || "An authorized family or associated-person match requires review");
      const relationships = this.database.prepare(`SELECT r.id,r.person_id AS personId,p.display_name AS personName,r.relationship_type AS relationshipType,
        r.representation_status AS representationStatus,r.status,r.description FROM professional_relationships r JOIN persons p ON p.id=r.person_id
        WHERE r.workspace_id=? AND r.entity_id=?`).all(workspaceId, hit.matchedEntityId);
      const partyConnections = this.database.prepare(`SELECT mp.id,mp.role,mp.status,m.id AS matterId,m.code,m.title,m.representation_status AS representationStatus
        FROM matter_parties mp JOIN matters m ON m.id=mp.matter_id WHERE m.workspace_id=? AND mp.entity_id=?`).all(workspaceId, hit.matchedEntityId);
      for (const relationship of relationships) {
        connections.push({ ...relationship, entityId:hit.matchedEntityId, entityName:hit.matchedEntityName, sourceType:"RELATIONSHIP" });
        const relationshipType = upper(relationship.relationshipType);
        const indicatorType = relationshipType === "FIDUCIARY_ROLE" ? "FIDUCIARY_RESPONSIBILITY"
          : relationshipType.includes("FAMILY") ? "FAMILY_CONNECTION"
          : relationshipType === "FINANCIAL_INTEREST" ? "FINANCIAL_INTEREST"
          : ["OUTSIDE_EMPLOYMENT","GIFT_OR_HOSPITALITY"].includes(relationshipType) ? "OUTSIDE_RESPONSIBILITY"
          : relationshipType === "FORMER_GOVERNMENT_EMPLOYMENT" ? "FORMER_GOVERNMENT_INTERSECTION" : null;
        if (indicatorType) addIndicator(indicatorType, hit, "RELATIONSHIP", relationship.id, relationship.description);
        if (["FORMER","HISTORICAL"].includes(upper(relationship.representationStatus)) || relationshipType === "FORMER_CLIENT") addIndicator("FORMER_CLIENT_INTERSECTION", hit, "RELATIONSHIP", relationship.id, relationship.description);
      }
      for (const party of partyConnections) {
        connections.push({ ...party, entityId:hit.matchedEntityId, entityName:hit.matchedEntityName, sourceType:"MATTER_PARTY" });
        const currentClient = party.role === "CLIENT" && ["ACTIVE","PROPOSED"].includes(upper(party.representationStatus));
        if (currentClient && ["ADVERSE_PARTY","OPPOSING_PARTY"].includes(subjectRole)) {
          addIndicator("CURRENT_CLIENT_ADVERSITY", hit, "MATTER_PARTY", party.id, `${party.code} records the matched entity as a current client`);
          if (party.matterId === matter?.id) addIndicator("SAME_MATTER_ADVERSE_POSITIONS", hit, "MATTER_PARTY", party.id, "The same matter records potentially adverse positions");
        }
        if (party.role === "FORMER_CLIENT") addIndicator("FORMER_CLIENT_INTERSECTION", hit, "MATTER_PARTY", party.id, `${party.code} records a former-client relationship`);
        if (party.role === "PROSPECTIVE_CLIENT") addIndicator("PROSPECTIVE_CLIENT_INTERSECTION", hit, "MATTER_PARTY", party.id, `${party.code} records a prospective-client relationship`);
      }
      if (hit.sourceResourceType === "LEDGER_ENTRY") {
        const ledger = this.database.prepare("SELECT id,involvement,context FROM personal_ledger_entries WHERE id=? AND sharing_authorized=1").get(hit.sourceResourceId);
        if (ledger) {
          connections.push({ ...ledger, entityId:hit.matchedEntityId, entityName:hit.matchedEntityName, sourceType:"LEDGER_ENTRY" });
          if (upper(ledger.involvement) === "FORMER_CLIENT") addIndicator("FORMER_CLIENT_INTERSECTION", hit, "LEDGER_ENTRY", ledger.id, ledger.context);
        }
      }
    }
    const derivedFacts = {};
    for (const [indicatorType, factId] of Object.entries(DERIVED_CONFLICT_FACTS)) if (indicators.some((indicator) => indicator.type === indicatorType)) derivedFacts[factId] = true;
    return {
      schema:"interlocks.policy-facts.v1", evaluationTime:at, workspaceId, corpusRevision:this.corpusRevision(),
      matter, matterParties, subjects:subjects.map(({ __policySubjectId, ...subject }) => ({ id:__policySubjectId, ...structuredClone(subject) })),
      hits:hits.map((hit) => ({ id:hit.id, subjectId:hit.subjectId, entityId:hit.matchedEntityId, entityName:hit.matchedEntityName, confidence:hit.matchConfidence, sourceType:hit.sourceResourceType, sourceId:hit.sourceResourceId })),
      connections, indicators, ...derivedFacts,
    };
  }

  recordPolicyQuestion(accountId, checkId, question, baseFacts, at) {
    const questionId = id("policy-question");
    this.database.prepare("INSERT INTO policy_questions (id,conflict_check_id,question_key,question_text,scope,created_by,created_at) VALUES (?,?,?,?, 'QUESTION',?,?)").run(questionId, checkId, question.key, question.text, accountId, at);
    const evaluations = [];
    for (const authority of question.authorities) {
      if (!["USER","RECOMMENDED","SYSTEM_FALLBACK"].includes(authority.source)) throw new Error("Unsupported policy selection source");
      const pack = getLegalPolicyPack(authority.packId);
      const selectionId = id("policy-selection");
      this.database.prepare(`INSERT INTO policy_authority_selections
        (id,question_id,pack_id,pack_version,authority_status,selection_source,rationale,selected_by,selected_at,pack_snapshot_json)
        VALUES (?,?,?,?,?,?,?,?,?,?)`).run(selectionId, questionId, pack.id, pack.version, authority.status, authority.source, authority.rationale, accountId, at, JSON.stringify(pack));
      const facts = { ...structuredClone(question.context), ...structuredClone(baseFacts) };
      if (pack.id === "delaware-chancery" && facts.tribunal == null) facts.tribunal = "DELAWARE_CHANCERY";
      const evaluation = evaluatePolicyPack(pack, facts, { authorityStatus:authority.status, evaluatedAt:at });
      const evaluationId = id("policy-evaluation"); const factsHash = policyContentHash(facts);
      this.database.prepare(`INSERT INTO policy_evaluations
        (id,question_id,authority_selection_id,engine_version,dsl_version,facts_hash,fact_snapshot_json,summary_json,evaluated_by,evaluated_at)
        VALUES (?,?,?,?,?,?,?,?,?,?)`).run(evaluationId, questionId, selectionId, evaluation.engineVersion, evaluation.dslVersion, factsHash, JSON.stringify(facts), JSON.stringify(evaluation.counts), accountId, at);
      for (const result of evaluation.results) this.database.prepare(`INSERT INTO policy_rule_results
        (id,evaluation_id,pack_id,pack_version,rule_id,corresponds_to,outcome,finding_code,finding_message,citation,source_url,comparison_note,missing_facts_json,unknown_questions_json,trace_json,created_at)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(id("policy-result"), evaluationId, pack.id, pack.version, result.ruleId, result.correspondsTo, result.outcome, result.finding?.code || null, result.finding?.message || null, result.citation, result.sourceUrl, result.comparisonNote, JSON.stringify(result.missingFacts), JSON.stringify(result.unknownQuestions), JSON.stringify(result.trace), at);
      const actionableResults = evaluation.results.filter((result) => authority.status !== "COMPARATIVE_ONLY" && result.outcome !== "NOT_MATCHED");
      evaluations.push({ id:evaluationId, questionId, packId:pack.id, packTitle:pack.title, packVersion:pack.version, authorityStatus:authority.status, counts:evaluation.counts, actionableResults });
    }
    return { id:questionId, key:question.key, text:question.text, evaluations, actionableResults:evaluations.flatMap((item) => item.actionableResults) };
  }

  createConflictCheck(accountId, workspaceId, input) {
    const actor = this.getActor(accountId); this.authorization.require(actor, ACTIONS.READ_WORKSPACE, { workspaceId });
    const subjects = (input.subjects || []).map((item) => structuredClone(item)); if (!subjects.length) throw new Error("At least one conflict-check subject is required");
    ...
    const policyQuestions = this.normalizePolicyQuestions(input);
    ...
    const visibleCandidates = this.visibleEntityCandidates(workspaceId, actor, input.participatingPersonIds || []);
    const visible = new Set(visibleCandidates.keys());
    const publicEntityIds = [...visibleCandidates].filter(([,candidate]) => candidate.sources.some((source) => !source.private)).map(([entityId]) => entityId).sort();
    const privateCandidateRecords = [...visibleCandidates].flatMap(([entityId,candidate]) => candidate.sources.filter((source) => source.private).map((source) => [entityId, source.sourceType, source.sourceId])).sort((left,right) => JSON.stringify(left).localeCompare(JSON.stringify(right)));
    const privateCandidateHash = createHash("sha256").update(JSON.stringify(privateCandidateRecords)).digest("hex");
    const knowledgeSnapshot = { corpusRevision:revision, entityIds:publicEntityIds, privateCandidateCount:privateCandidateRecords.length, privateCandidateHash };
    const candidates = [...visibleCandidates].map(([entityId,candidate]) => {
      ...
      return { ...entity, sources:candidate.sources };
    });
    const createdHits = []; const policySummaries = [];
    transaction(this.database, () => {
      this.database.prepare("INSERT INTO conflict_checks (id,workspace_id,matter_id,reference,status,workflow_state,created_by,executed_at,corpus_revision,subjects_snapshot_json,knowledge_snapshot_json,completed_at) VALUES (?,?,?,?,'COMPLETE','GREEN',?,?,?,?,?,?)").run(checkId, workspaceId, clean(input.matterId) || null, reference, accountId, at, revision, JSON.stringify(subjects), JSON.stringify(knowledgeSnapshot), at);
      for (const subject of subjects) {
        const subjectId = id("subject");
        subject.__policySubjectId = subjectId;
        ...
        for (const candidate of candidates) {
          ...
          if (!result) continue; directMatches.push(candidate.id); createdHits.push(this.insertHit(checkId, subjectId, candidate.id, result, at, candidate.sources));
        }
        for (const matchedId of directMatches) {
          ...
          for (const relation of relations) {
            ...
            createdHits.push(this.insertHit(checkId, subjectId, relatedId, relatedMatch({ type: relation.relationship_type, fromName: relation.fromName, toName: relation.toName }), at, [{ sourceType:"ENTITY_RELATIONSHIP", sourceId:relation.id, private:false, priority:80 }]));
          }
        }
      }
      const baseFacts = this.buildPolicyFacts(workspaceId, input, subjects, createdHits, at);
      for (const question of policyQuestions) policySummaries.push(this.recordPolicyQuestion(accountId, checkId, question, baseFacts, at));
      const policyActionCount = policySummaries.reduce((count, question) => count + question.actionableResults.length, 0);
      this.database.prepare("UPDATE conflict_checks SET workflow_state=?,knowledge_snapshot_json=? WHERE id=?").run(createdHits.length || policyActionCount ? "YELLOW" : "GREEN", JSON.stringify({ ...knowledgeSnapshot, policyFactsHash:policyContentHash(baseFacts), policyQuestionKeys:policyQuestions.map((item) => item.key) }), checkId);
      ...
      if (!createdHits.length) for (const question of policySummaries.filter((item) => item.actionableResults.length)) {
        const caseId = id("case"); const caseReference = this.nextCaseReference(workspaceId);
        const messages = [...new Set(question.actionableResults.map((result) => result.finding?.message).filter(Boolean))];
        this.database.prepare(`INSERT INTO review_cases
          (id,workspace_id,conflict_check_id,policy_question_id,reference,matter_id,title,summary,workflow_state,human_disposition,status,assigned_account_id,opened_at,review_due_at,updated_at)
          VALUES (?,?,?,?,?,?,?,?,'YELLOW','UNREVIEWED','NEW',?,?,?,?)`).run(caseId, workspaceId, checkId, question.id, caseReference, clean(input.matterId) || null, `Review policy question: ${question.text}`, messages.join("; ") || "Additional facts are required before the selected policy packs can be evaluated.", accountId, at, addDays(7), at);
      }
      this.audit(actor, { authority: "MEMBER", workspaceId, action: "conflict_check.executed", resourceType: "CONFLICT_CHECK", resourceId: checkId, after: { reference, corpusRevision: revision, hitCount: createdHits.length, policyQuestionCount:policyQuestions.length, policyActionCount } });
    });
    const policyActionCount = policySummaries.reduce((count, question) => count + question.actionableResults.length, 0);
    return { id: checkId, reference, workflowState: createdHits.length || policyActionCount ? "YELLOW" : "GREEN", hits: createdHits, policyQuestions:policySummaries };
  }

  insertHit(checkId, subjectId, matchedEntityId, result, at, sources = []) {
    const entity = this.database.prepare("SELECT canonical_name AS name FROM entities WHERE id=?").get(matchedEntityId);
    const hitId = id("hit"); const evidenceSources = sources.length ? sources : [{ sourceType:"ENTITY", sourceId:matchedEntityId, private:false, priority:999 }];
    const primarySource = evidenceSources[0]; const effectiveSourceType = primarySource.sourceType; const effectiveSourceId = primarySource.sourceId;
    const sourceReasons = [...new Set(evidenceSources.map((source) => source.sourceDetail).filter(Boolean))];
    const reasons = [...result.reasons, ...sourceReasons];
    const explanation = { reasons, source: effectiveSourceType, statement: "Candidate surfaced for human review; no legal conclusion has been made." };
    ...
    for (const source of evidenceSources) this.database.prepare("INSERT INTO conflict_hit_evidence (id,conflict_hit_id,evidence_type,evidence_id,explanation) VALUES (?,?,?,?,?)").run(id("hit-evidence"), hitId, source.sourceType, source.sourceId, source.sourceDetail || result.reasons.join("; "));
    const hit = { id: hitId, subjectId, matchedEntityId, matchedEntityName: entity.name, matchConfidence: result.confidence, sourceResourceType:effectiveSourceType, sourceResourceId:effectiveSourceId, explanation };
    Object.defineProperty(hit, "__evidenceSources", { value:evidenceSources, enumerable:false });
    return hit;
  }
  ...
  recordCaseAction(caseId, action, accountId = "acct-alex") {
    ...
    transaction(this.database, () => {
      ...
      if (action.type === "status") {
        const status = upper(required(action.status, "Status"));
        if (!["NEW","IN_REVIEW","AWAITING_RESPONSE","MANAGED","CLOSED","DECLINED","WITHDRAWN"].includes(status)) throw new Error("Unsupported case status");
        this.database.prepare("UPDATE review_cases SET status=?,updated_at=? WHERE id=?").run(status, at, caseId);
  ...
  createConsent(accountId, caseId, input) {
    ...
    transaction(this.database, () => {
      ...
      for (const documentId of input.documentIds || []) this.attachDocumentInternal(documentId, { resourceType: "CONSENT", resourceId: consentId, role: "CONSENT_EVIDENCE" }, accountId, item.workspaceId, at);
  ...
  createScreen(accountId, caseId, input) {
    ...
    transaction(this.database, () => {
      ...
      for (const documentId of input.documentIds || []) this.attachDocumentInternal(documentId, { resourceType: "SCREEN", resourceId: screenId, role: "SCREEN_EVIDENCE" }, accountId, item.workspaceId, at);
  ...
  createPersonalAssociation(accountId, input) {
    const actor = this.getActor(accountId);
    this.authorization.require(actor, ACTIONS.SHARE_PERSONAL_LEDGER, { ownerPersonId: actor.personId });
    const name = required(input.associatedPersonName, "Associated person name");
    const relationshipType = assertFamilyRelationship(input.relationshipType);
    const { start, end } = dateWindow(input.effectiveFrom, input.effectiveTo);
    const associationId = id("personal-association"); const associatedPersonId = id("person"); const associatedEntityId = id("entity"); const at = isoNow();
    transaction(this.database, () => {
      this.database.prepare("INSERT INTO persons (id,display_name,primary_profession,created_at,updated_at) VALUES (?,?,?,?,?)").run(associatedPersonId, name, clean(input.primaryProfession) || null, at, at);
      this.database.prepare("INSERT INTO entities (id,kind,canonical_name,person_id,created_at,updated_at) VALUES (?,'PERSON',?,?,?,?)").run(associatedEntityId, name, associatedPersonId, at, at);
      const disclosureScope = assertAssociationDisclosureScope(input.disclosureScope);
      this.database.prepare(`INSERT INTO personal_associations
        (id,owner_person_id,associated_person_id,relationship_type,disclosure_scope,status,effective_from,effective_to,provenance,created_by,created_at,updated_at)
        VALUES (?,?,?,?,?,'ACTIVE',?,?,?,?,?,?)`).run(associationId, actor.personId, associatedPersonId, relationshipType, disclosureScope, start, end, required(input.provenance, "Provenance"), accountId, at, at);
      this.incrementPersonalCorpus();
      this.audit(actor, { authority:"ACCOUNT_OWNER", action:"personal_association.created", resourceType:"PERSONAL_ASSOCIATION", resourceId:associationId, after:{ relationshipType, disclosureScope } });
    });
    this.rebuildSearchIndex();
    return { id:associationId, associatedPersonId, associatedPersonName:name, relationshipType, status:"ACTIVE" };
  }

  endPersonalAssociation(accountId, associationId) {
    const actor = this.getActor(accountId);
    this.authorization.require(actor, ACTIONS.SHARE_PERSONAL_LEDGER, { ownerPersonId:actor.personId });
    const association = this.database.prepare("SELECT * FROM personal_associations WHERE id=? AND owner_person_id=?").get(associationId, actor.personId);
    if (!association) throw new Error("Personal association not found");
    if (association.status !== "ACTIVE") throw new Error("Personal association is not active");
    const at = isoNow();
    transaction(this.database, () => {
      this.database.prepare("UPDATE personal_associations SET status='ENDED',effective_to=COALESCE(effective_to,?),updated_at=? WHERE id=?").run(at.slice(0, 10), at, associationId);
      this.database.prepare("UPDATE personal_association_interests SET status='ENDED',effective_to=COALESCE(effective_to,?) WHERE association_id=? AND status='CURRENT'").run(at.slice(0, 10), associationId);
      this.incrementPersonalCorpus();
      this.audit(actor, { authority:"ACCOUNT_OWNER", action:"personal_association.ended", resourceType:"PERSONAL_ASSOCIATION", resourceId:associationId, before:{ status:"ACTIVE" }, after:{ status:"ENDED" } });
    });
    return { id:associationId, status:"ENDED" };
  }

  createAssociationInterest(accountId, associationId, input) {
    const actor = this.getActor(accountId);
    this.authorization.require(actor, ACTIONS.SHARE_PERSONAL_LEDGER, { ownerPersonId:actor.personId });
    const association = this.database.prepare("SELECT * FROM personal_associations WHERE id=? AND owner_person_id=?").get(associationId, actor.personId);
    if (!association) throw new Error("Personal association not found");
    if (association.status !== "ACTIVE") throw new Error("Personal association is not active");
    const entityName = required(input.entityName, "Connected entity name");
    const { start, end } = dateWindow(input.effectiveFrom, input.effectiveTo);
    const allowedKinds = ["PERSON","ORGANIZATION","PROPERTY","TRUST","ESTATE","GOVERNMENT_BODY","OTHER"];
    const kind = upper(input.entityKind || "ORGANIZATION"); if (!allowedKinds.includes(kind)) throw new Error("Unsupported entity kind");
    let entity = this.database.prepare("SELECT id FROM entities WHERE lower(canonical_name)=lower(?) ORDER BY rowid LIMIT 1").get(entityName);
    const entityId = entity?.id || id("entity"); const interestId = id("association-interest"); const at = isoNow();
    transaction(this.database, () => {
      if (!entity) this.database.prepare("INSERT INTO entities (id,kind,canonical_name,jurisdiction,created_at,updated_at) VALUES (?,?,?,?,?,?)").run(entityId, kind, entityName, clean(input.jurisdiction) || null, at, at);
      this.database.prepare(`INSERT INTO personal_association_interests
        (id,association_id,entity_id,involvement,description,status,sharing_authorized,effective_from,effective_to,recorded_by,recorded_at)
        VALUES (?,?,?,?,?,'CURRENT',1,?,?,?,?)`).run(interestId, associationId, entityId, upper(required(input.involvement, "Involvement")), required(input.description, "Description"), start, end, accountId, at);
      this.incrementPersonalCorpus();
      this.audit(actor, { authority:"ACCOUNT_OWNER", action:"association_interest.created", resourceType:"ASSOCIATION_INTEREST", resourceId:interestId, after:{ associationId, entityId, involvement:upper(input.involvement) } });
    });
    if (!entity) this.rebuildSearchIndex();
    return { id:interestId, associationId, entityId, entityName, status:"CURRENT", sharingAuthorized:true };
  }

  revokeAssociationInterest(accountId, interestId) {
    const actor = this.getActor(accountId);
    this.authorization.require(actor, ACTIONS.SHARE_PERSONAL_LEDGER, { ownerPersonId:actor.personId });
    const interest = this.database.prepare(`SELECT pai.* FROM personal_association_interests pai JOIN personal_associations pa ON pa.id=pai.association_id
      WHERE pai.id=? AND pa.owner_person_id=?`).get(interestId, actor.personId);
    if (!interest) throw new Error("Association interest not found");
    if (interest.status !== "CURRENT") throw new Error("Association interest is not current");
    const at = isoNow();
    transaction(this.database, () => {
      this.database.prepare("UPDATE personal_association_interests SET status='REVOKED',sharing_authorized=0,revoked_at=? WHERE id=?").run(at, interestId);
      this.incrementPersonalCorpus();
      this.audit(actor, { authority:"ACCOUNT_OWNER", action:"association_interest.revoked", resourceType:"ASSOCIATION_INTEREST", resourceId:interestId, before:{ status:"CURRENT", sharingAuthorized:true }, after:{ status:"REVOKED", sharingAuthorized:false } });
    });
    return { id:interestId, status:"REVOKED", sharingAuthorized:false };
  }

  requestFamilyAccountLink(accountId, input) {
    const actor = this.getActor(accountId);
    this.authorization.require(actor, ACTIONS.SHARE_PERSONAL_LEDGER, { ownerPersonId:actor.personId });
    const email = required(input.targetEmail, "Linked account email").toLowerCase();
    const target = this.database.prepare("SELECT a.id AS accountId,a.person_id AS personId FROM accounts a WHERE lower(a.primary_email)=? AND a.status='ACTIVE'").get(email);
    if (!target || target.personId === actor.personId) throw new Error("No eligible Interlocks account can accept this link");
    const relationshipType = assertFamilyRelationship(input.relationshipType);
    const pairKey = canonicalFamilyPair(actor.personId, target.personId);
    if (this.database.prepare("SELECT 1 FROM family_account_links WHERE pair_key=? AND status IN ('PENDING','ACTIVE')").get(pairKey)) throw new Error("An open family account link already exists between these people");
    const expiresInDays = Number(input.expiresInDays || 14); if (!Number.isInteger(expiresInDays) || expiresInDays < 1 || expiresInDays > 90) throw new Error("Link invitation expiry must be between 1 and 90 days");
    const linkId = id("family-link"); const at = isoNow();
    transaction(this.database, () => {
      this.database.prepare(`INSERT INTO family_account_links
        (id,pair_key,requester_person_id,target_person_id,relationship_type,target_relationship_type,disclosure_scope,status,requested_by,requested_at,expires_at)
        VALUES (?,?,?,?,?,?,'ENTITY_MATCH_ONLY','PENDING',?,?,?)`).run(linkId, pairKey, actor.personId, target.personId, relationshipType, reciprocalFamilyRelationship(relationshipType), accountId, at, addDays(expiresInDays));
      this.audit(actor, { authority:"ACCOUNT_OWNER", action:"family_account_link.requested", resourceType:"FAMILY_ACCOUNT_LINK", resourceId:linkId, after:{ relationshipType, disclosureScope:"ENTITY_MATCH_ONLY" } });
    });
    return { id:linkId, status:"PENDING", relationshipType, disclosureScope:"ENTITY_MATCH_ONLY" };
  }

  respondFamilyAccountLink(accountId, linkId, input) {
    const actor = this.getActor(accountId);
    this.authorization.require(actor, ACTIONS.SHARE_PERSONAL_LEDGER, { ownerPersonId:actor.personId });
    const link = this.database.prepare("SELECT * FROM family_account_links WHERE id=?").get(linkId);
    if (!link || link.target_person_id !== actor.personId) throw new Error("Family account link not found");
    if (link.status !== "PENDING") throw new Error("Family account link is not awaiting a response");
    if (link.expires_at && new Date(link.expires_at) <= new Date()) {
      const at = isoNow();
      transaction(this.database, () => {
        this.database.prepare("UPDATE family_account_links SET status='EXPIRED',responded_by=?,responded_at=? WHERE id=?").run(accountId, at, linkId);
        this.audit(actor, { authority:"ACCOUNT_OWNER", action:"family_account_link.expired", resourceType:"FAMILY_ACCOUNT_LINK", resourceId:linkId, before:{ status:"PENDING" }, after:{ status:"EXPIRED" } });
      });
      throw new Error("Family account link invitation has expired");
    }
    const response = upper(input.response); if (!["ACCEPT","DECLINE"].includes(response)) throw new Error("Unsupported family account link response");
    const status = response === "ACCEPT" ? "ACTIVE" : "DECLINED"; const at = isoNow();
    transaction(this.database, () => {
      this.database.prepare("UPDATE family_account_links SET status=?,responded_by=?,responded_at=?,accepted_at=? WHERE id=?").run(status, accountId, at, status === "ACTIVE" ? at : null, linkId);
      if (status === "ACTIVE") this.incrementPersonalCorpus();
      this.audit(actor, { authority:"ACCOUNT_OWNER", action:`family_account_link.${status === "ACTIVE" ? "accepted" : "declined"}`, resourceType:"FAMILY_ACCOUNT_LINK", resourceId:linkId, before:{ status:"PENDING" }, after:{ status } });
    });
    return { id:linkId, status };
  }

  revokeFamilyAccountLink(accountId, linkId) {
    const actor = this.getActor(accountId);
    this.authorization.require(actor, ACTIONS.SHARE_PERSONAL_LEDGER, { ownerPersonId:actor.personId });
    const link = this.database.prepare("SELECT * FROM family_account_links WHERE id=? AND (requester_person_id=? OR target_person_id=?)").get(linkId, actor.personId, actor.personId);
    if (!link) throw new Error("Family account link not found");
    if (link.status !== "ACTIVE") throw new Error("Family account link is not active");
    const at = isoNow();
    transaction(this.database, () => {
      this.database.prepare("UPDATE family_account_links SET status='REVOKED',revoked_by=?,revoked_at=? WHERE id=?").run(accountId, at, linkId);
      this.incrementPersonalCorpus();
      this.audit(actor, { authority:"ACCOUNT_OWNER", action:"family_account_link.revoked", resourceType:"FAMILY_ACCOUNT_LINK", resourceId:linkId, before:{ status:"ACTIVE" }, after:{ status:"REVOKED" } });
    });
  ...
  respondAssociatedPerson(accountId, requestId, input) {
    ...
    if (actor.personId !== request.subject_person_id) this.authorization.require(actor, ACTIONS.REVIEW, { workspaceId: request.workspace_id });
    if (request.status !== "PENDING") throw new Error("Associated-person request was already answered");
    if (request.expires_at && new Date(request.expires_at) <= new Date()) throw new Error("Associated-person request has expired");
  ...
  previewImport(accountId, workspaceId, input) {
    ...
    if (!rows.length) errors.push({ row: 1, message: "At least one data row is required" });
  ...
  exportData(accountId = "acct-alex", workspaceId = "ws-northstar", kind = "workspace", resourceId = null) {
    ...
    if (kind === "personal") {
      ...
      const associations = this.database.prepare(`SELECT pa.*,p.display_name AS associated_person_name FROM personal_associations pa JOIN persons p ON p.id=pa.associated_person_id
        WHERE pa.owner_person_id=? ORDER BY pa.created_at`).all(actor.personId);
      const associationInterests = this.database.prepare(`SELECT pai.*,e.canonical_name AS entity_name FROM personal_association_interests pai
        JOIN personal_associations pa ON pa.id=pai.association_id JOIN entities e ON e.id=pai.entity_id WHERE pa.owner_person_id=? ORDER BY pai.recorded_at`).all(actor.personId);
      const familyAccountLinks = this.database.prepare(`SELECT fl.id,fl.requester_person_id,fl.target_person_id,
        CASE WHEN fl.requester_person_id=? THEN target.display_name ELSE requester.display_name END AS other_person_name,
        CASE WHEN fl.requester_person_id=? THEN fl.relationship_type ELSE fl.target_relationship_type END AS relationship_type,
        fl.disclosure_scope,fl.status,fl.requested_at,fl.expires_at,fl.accepted_at,fl.revoked_at
        FROM family_account_links fl JOIN persons requester ON requester.id=fl.requester_person_id JOIN persons target ON target.id=fl.target_person_id
        WHERE fl.requester_person_id=? OR fl.target_person_id=? ORDER BY fl.requested_at`).all(actor.personId, actor.personId, actor.personId, actor.personId);
      this.audit(actor, { authority: "ACCOUNT_OWNER", action: "personal_ledger.exported", resourceType: "PERSON", resourceId: actor.personId });
      return { schema: "interlocks.personal-ledger.v2", exportedAt: isoNow(), person: { id: actor.personId, name: actor.name }, entries, associations, associationInterests, familyAccountLinks, linkedAccountLedgersIncluded:false };
    }
    ...
    if (kind === "check") {
      ...
      result = { schema: "interlocks.conflict-check.v2", check,
        subjects: this.database.prepare("SELECT * FROM conflict_check_subjects WHERE conflict_check_id=?").all(resourceId),
        hits: this.database.prepare("SELECT * FROM conflict_hits WHERE conflict_check_id=?").all(resourceId),
        hitEvidence: this.database.prepare("SELECT he.* FROM conflict_hit_evidence he JOIN conflict_hits h ON h.id=he.conflict_hit_id WHERE h.conflict_check_id=? ORDER BY he.conflict_hit_id,he.evidence_type,he.evidence_id").all(resourceId),
        policyQuestions: this.database.prepare("SELECT * FROM policy_questions WHERE conflict_check_id=?").all(resourceId),
        policySelections: this.database.prepare("SELECT s.* FROM policy_authority_selections s JOIN policy_questions q ON q.id=s.question_id WHERE q.conflict_check_id=?").all(resourceId),
        policyEvaluations: this.database.prepare("SELECT pe.* FROM policy_evaluations pe JOIN policy_questions q ON q.id=pe.question_id WHERE q.conflict_check_id=?").all(resourceId),
        policyRuleResults: this.database.prepare("SELECT pr.* FROM policy_rule_results pr JOIN policy_evaluations pe ON pe.id=pr.evaluation_id JOIN policy_questions q ON q.id=pe.question_id WHERE q.conflict_check_id=?").all(resourceId),
        cases: this.database.prepare("SELECT * FROM review_cases WHERE conflict_check_id=?").all(resourceId),
        determinations: this.database.prepare("SELECT d.* FROM human_determinations d JOIN review_cases c ON c.id=d.case_id WHERE c.conflict_check_id=?").all(resourceId),
        consents: this.database.prepare("SELECT co.* FROM conflict_consents co JOIN review_cases c ON c.id=co.case_id WHERE c.conflict_check_id=?").all(resourceId),
        screens: this.database.prepare("SELECT s.* FROM screens s JOIN review_cases c ON c.id=s.case_id WHERE c.conflict_check_id=?").all(resourceId), exportedAt: isoNow() };
    } else {
      const snapshot = this.getSnapshot(accountId, workspaceId);
      const { ledger:personalLedger, personalAssociations, associationInterests, familyAccountLinks, ...workspaceSnapshot } = snapshot;
      void personalLedger; void personalAssociations; void associationInterests; void familyAccountLinks;
      result = { schema: "interlocks.workspace.v1", ...workspaceSnapshot, personalDataExcluded:true };
    }
  ...
  resetDemo(accountId = "acct-alex") {
    ...
    const tables = ["audit_events","seat_events","import_batches","associated_person_responses","associated_person_requests","controls","screens","conflict_consents","human_determinations","review_notes","review_cases","policy_rule_results","policy_evaluations","policy_authority_selections","policy_questions","conflict_hit_evidence","conflict_hits","conflict_check_subjects","conflict_checks","evidence_links","resource_attachments","documents","inferences","assertions","personal_association_interests","personal_associations","family_account_links","personal_ledger_entries","professional_relationships","matter_parties","matters","invitations","workspace_roles","workspace_memberships","workspaces","entity_relationships","entity_addresses","entity_identifiers","entity_aliases","entities","auth_identities","accounts","persons"];
    transaction(this.database, () => {
