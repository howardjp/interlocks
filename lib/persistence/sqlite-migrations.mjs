const RESOURCE_TYPES = [
  ...
  "ASSOCIATED_PERSON_REQUEST", "PERSONAL_ASSOCIATION", "ASSOCIATION_INTEREST", "FAMILY_ACCOUNT_LINK",
  "IMPORT_BATCH", "ADMINISTRATIVE_ACTION",
];
    ...
    up(db) {
      db.exec(`
        CREATE TABLE personal_associations (
          ...
          associated_person_id TEXT NOT NULL REFERENCES persons(id),
          ...
          disclosure_scope TEXT NOT NULL DEFAULT 'MATCH_AND_RELATIONSHIP' CHECK (disclosure_scope IN ('CONFLICT_CHECK_ONLY','MATCH_AND_RELATIONSHIP')),
          status TEXT NOT NULL DEFAULT 'ACTIVE' CHECK (status IN ('ACTIVE','ENDED')),
          effective_from TEXT, effective_to TEXT, provenance TEXT NOT NULL,
          created_by TEXT NOT NULL REFERENCES accounts(id), created_at TEXT NOT NULL, updated_at TEXT NOT NULL,
          CHECK (owner_person_id != associated_person_id)
        );
        CREATE TABLE personal_association_interests (
          id TEXT PRIMARY KEY,
          association_id TEXT NOT NULL REFERENCES personal_associations(id),
          entity_id TEXT NOT NULL REFERENCES entities(id), involvement TEXT NOT NULL, description TEXT NOT NULL,
          status TEXT NOT NULL DEFAULT 'CURRENT' CHECK (status IN ('CURRENT','REVOKED','ENDED')),
          sharing_authorized INTEGER NOT NULL DEFAULT 1 CHECK (sharing_authorized IN (0,1)),
          effective_from TEXT, effective_to TEXT, recorded_by TEXT NOT NULL REFERENCES accounts(id),
          recorded_at TEXT NOT NULL, revoked_at TEXT
        );
        CREATE TABLE family_account_links (
          id TEXT PRIMARY KEY, pair_key TEXT NOT NULL,
          requester_person_id TEXT NOT NULL REFERENCES persons(id), target_person_id TEXT NOT NULL REFERENCES persons(id),
          relationship_type TEXT NOT NULL CHECK (relationship_type IN ('SPOUSE','DOMESTIC_PARTNER','PARENT','CHILD','SIBLING','GRANDPARENT','GRANDCHILD','IN_LAW','HOUSEHOLD_MEMBER','DEPENDENT','GUARDIAN','CLOSE_PERSONAL_RELATIONSHIP','OTHER')),
          ...
          requested_by TEXT NOT NULL REFERENCES accounts(id), requested_at TEXT NOT NULL, expires_at TEXT,
          responded_by TEXT REFERENCES accounts(id), responded_at TEXT, accepted_at TEXT,
          revoked_by TEXT REFERENCES accounts(id), revoked_at TEXT,
          CHECK (requester_person_id != target_person_id)
        );
        CREATE INDEX idx_personal_association_owner ON personal_associations(owner_person_id,status);
        CREATE INDEX idx_association_interest_association ON personal_association_interests(association_id,status);
        CREATE INDEX idx_association_interest_entity ON personal_association_interests(entity_id,status);
        CREATE INDEX idx_family_link_requester ON family_account_links(requester_person_id,status);
        CREATE INDEX idx_family_link_target ON family_account_links(target_person_id,status);
        CREATE UNIQUE INDEX idx_family_link_open_pair ON family_account_links(pair_key) WHERE status IN ('PENDING','ACTIVE');
      `);
    },
  },
];
