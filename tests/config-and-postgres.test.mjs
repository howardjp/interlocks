...
test("PostgreSQL cutover has ordered native migrations for every critical aggregate", () => {
  assert.equal(postgresCutover.status,"migration-ready"); assert.deepEqual(postgresMigrations.map((item)=>item.version),[1]);
  const sql=postgresMigrations[0].sql;
  for(const table of ["persons","accounts","workspaces","workspace_memberships","entities","matters","personal_ledger_entries","documents","assertions","inferences","conflict_checks","conflict_hits","review_cases","human_determinations","conflict_consents","screens","audit_events"]) assert.match(sql,new RegExp(`CREATE TABLE ${table}`));
